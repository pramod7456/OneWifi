...
static int lq_ipc_open_fd(void)
{
    ...
    if (lq_ipc_fd < 0) {
        wifi_util_error_print(WIFI_APPS, "%s:%d socket() failed: %s\n", __func__, __LINE__,
            strerror(errno));
        return -1;
    }
...
static void lq_ipc_log_stats_entries(uint32_t msg_type, const void *entries, uint32_t count,
    size_t entry_size)
{
    ...
    for (uint32_t i = 0; i < count; i++) {
        wifi_util_dbg_print(WIFI_APPS,
            ...
            __func__, lq_msg_type_str(msg_type), i + 1, count, s[i].mac_str, s[i].status_code,
            s[i].event, (long long)s[i].total_connected_time.tv_sec,
            (long long)s[i].total_disconnected_time.tv_sec, s[i].dev.cli_SNR, s[i].vap_index,
            s[i].radio_index);
 ...
 */
static int build_tlv(uint32_t msg_type, const void *entries, uint32_t count, size_t entry_size,
    uint8_t *buf, size_t buf_sz)
{
    ...
    if (needed > buf_sz) {
        wifi_util_error_print(WIFI_APPS, "%s:%d [TLV] buffer too small: need %zu have %zu\n",
            __func__, __LINE__, needed, buf_sz);
        return -1;
    }

    if (data_sz > UINT16_MAX) {
        wifi_util_error_print(WIFI_APPS, "%s:%d [TLV] payload %zu exceeds uint16_t max\n", __func__,
            __LINE__, data_sz);
        return -1;
    }
    ...
    wifi_util_dbg_print(WIFI_APPS, "%s:%d [TLV] msg_type=%u data_sz=%zu tlv_bytes=%zu\n", __func__,
        __LINE__, msg_type, data_sz, sizeof(lq_tlv_t) + data_sz);
...
int lq_ipc_send(uint32_t msg_type, const void *entries, uint32_t count, size_t entry_size)
{
            ...
            if (mgr != NULL) {
                ...
                for (uint32_t i = 0; i < count; i++) {
                    if ((is_vap_private(&mgr->hal_cap.wifi_prop, s[i].vap_index) != TRUE) &&
                        ...
                        wifi_util_dbg_print(WIFI_APPS,
                            "%s:%d [IPC-SEND] dropping %s: vap_index=%u is not a private/station "
                            "VAP\n",
    ...
    if (!buf) {
        wifi_util_error_print(WIFI_APPS, "%s:%d malloc(%zu) failed\n", __func__, __LINE__,
            alloc_sz);
        return -1;
    }
    ...
    wifi_util_dbg_print(WIFI_APPS,
        ...
        __LINE__, lq_msg_type_str(msg_type), msg_type, tlv_len, (size_t)tlv_len);
    ...
    for (int attempt = 0; attempt < 2; attempt++) {
        ret = sendto(lq_ipc_fd, buf, (size_t)tlv_len, MSG_DONTWAIT, (struct sockaddr *)&addr,
            sizeof(addr));
        if (ret >= 0) {
            wifi_util_info_print(WIFI_APPS,
                "%s:%d [IPC-SEND] %s sent %zd bytes OK (count=%u attempt=%d)\n", __func__, __LINE__,
                lq_msg_type_str(msg_type), ret, count, attempt);
            break;
        }

        int err = errno;
        wifi_util_dbg_print(WIFI_APPS, "%s:%d [IPC-SEND] sendto(%s) failed: %s (attempt %d)\n",

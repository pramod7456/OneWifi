...
static int periodic_caffinity_stats_update_ext(stats_arg_t *stats, int len)
{
    ...
    if (access(QMGR_FILE, F_OK) == 0) {
        ...
        if (g_gw_mac[0] == 0 && g_gw_mac[1] == 0 && g_gw_mac[2] == 0 &&
            g_gw_mac[3] == 0 && g_gw_mac[4] == 0 && g_gw_mac[5] == 0) {
...
int lq_send_autoconf_search(const char *ifname)
{
    ...
    return send_frame(buf, (unsigned int)frame_len, false, (char *)ifname);
}
...
void lq_handle_1905_frame(const uint8_t *buf, ssize_t len)
{
    ...
    wifi_util_info_print(WIFI_APPS,
        " 1905 GW %s:%d dispatching event=%d count=%d\n",

...
{
    wifi_util_info_print(WIFI_BUS, "%s:%d string value =%s\n", __func__, __LINE__, str);
    ...
    wifi_util_info_print(WIFI_BUS, "%s:%d modified string value =%s\n", __func__, __LINE__,
        modified);
}
...
{
    wifi_util_info_print(WIFI_BUS, "Enter %s:%d:type=%d\n", __func__, __LINE__, type);
    switch (type) {
    ...
    default:
        wifi_util_info_print(WIFI_BUS, "%s:%d: bus: Invalid data_type for .\n", __func__, __LINE__);
        break;
    }
    data->data_type = type;
}
static char *convert_raw_data_to_dbus_message(raw_data_t *data, int *type)
{
    ...
    case bus_data_type_uint32:
        *type = ccsp_unsignedInt;
        ...
        break;
    default:
        wifi_util_info_print(WIFI_BUS, "%s:%d: bus: Invalid data_type:%d.\n", __func__, __LINE__,
            data->data_type);
        break;
    };
    return str;
    ...
    void *user_data)
{
    ...
    if (!handle) {
        wifi_util_info_print(WIFI_BUS, "handle is NULL returning\n");
    ...
    if (!reply) {
        wifi_util_info_print(WIFI_BUS, "Out of memory!\n");
    ...
    if (!strcmp(DBUS_INTERFACE_BASE, interface) && !strcmp("setParameterValues", method_name)) {
        ...
        if (!dbus_connection_send(conn, reply, NULL))
            wifi_util_info_print(WIFI_BUS, "No memory\n");
        ...
        if (parameterNames) {
            ...
            dbus_message_iter_init_append(reply, &iter);
            wifi_util_info_print(WIFI_BUS, "<%s> no parameter\n", __func__);
            if (!dbus_connection_send(conn, reply, NULL))
        ...
        if (result == 0) {
            ...
            wifi_util_info_print(WIFI_BUS, " %s:%d int_str=%s and type=%d\n", __func__, __LINE__,
                int_str, type);
            DBUS_MESSAGE_APPEND_STRING_1(&struct_iter, parameterNames);
        ...
        if (table_name) {
            ...
            dbus_message_iter_init_append(reply, &iter);
            wifi_util_info_print(WIFI_BUS, "<%s :%d> no parameter\n", __func__, __LINE__);
            if (!dbus_connection_send(conn, reply, NULL))
                wifi_util_info_print(WIFI_BUS, "<%s> No memory\n", __func__);
        ...
        if (result == 0) {
            ...
            wifi_util_info_print(WIFI_BUS, " %s:%d int_str=%s and type=%d\n", __func__, __LINE__,
                int_str, type1);
            DBUS_MESSAGE_APPEND_STRING_1(&struct_iter, val->parameterName);
            dbus_message_iter_append_basic(&struct_iter, DBUS_TYPE_STRING, &int_str);
    ...
    bus_event_sub_action_t action, int32_t interval, bool *autoPublish)
{
    ...
    if ((rc != bus_error_success) || !dbus_path || !dst_component_id) {
        wifi_util_info_print(WIFI_BUS, "Destination component not found\n");
        return bus_error_invalid_input;
    }
    ...
    DBUS_MESSAGE_APPEND_STRING_1(&iter, event_name);
...
static bus_error_t add_subscription_to_dbus_daemon(bus_handle_t *handle, const char *event_name)
{
    ...
    wifi_util_info_print(WIFI_BUS, "Match added to %s\n", tmp);
...
bus_error_t append_subscriber_events(bus_handle_t *handle, void *cb, char const *event_name)
{
    ...
    if ((callback = (bus_sub_callback_table_t *)hash_map_get(handle->subscribe_callback,
        ...
        wifi_util_info_print(WIFI_BUS, "Entered here\n");
...
bus_error_t remove_method_callback(bus_handle_t *handle, bus_data_element_t *element)
{
    ...
    wifi_util_info_print(WIFI_BUS, "%s:Exit %d\n", __func__, __LINE__);
    return bus_error_success;
}
...
DBusHandlerResult signal_handler(DBusConnection *connection, DBusMessage *msg, void *user_data)
{
    ...
    wifi_util_info_print(WIFI_BUS, "%s:Enter %d and dbus connection is %p\n", __func__, __LINE__,
        connection);
...
static bus_error_t check_dbus_handlers(bus_handle_t *handle, char *dbus_path)
{
    ...
    if (server_desc == NULL) {
        wifi_util_info_print(WIFI_BUS, "server_desc is null\n");
        return bus_error_success;
    }
    if (handle == NULL) {
        // Handle the error
    ...
    if (server == NULL) {
        wifi_util_info_print(WIFI_BUS, "This component is not present hence can add\n");
        return bus_error_success;
    }
...
{
    wifi_util_info_print(WIFI_BUS, "[DL]%s:%d\n", __func__, __LINE__);
...
static bus_error_t bus_open(bus_handle_t *handle, char *component_name)
{
    ...
    if (rc != bus_error_success) {
        wifi_util_info_print(WIFI_BUS, "the handler is %p\n", handle->u.dbus_handle);
        return bus_error_success;
    }
    handle->u.dbus_handle = dbus_bus_get_private(DBUS_BUS_SYSTEM, &dbus_error);
    if (dbus_error_is_set(&dbus_error)) {
        wifi_util_info_print(WIFI_BUS, "dbus_bus_get failed with %s\n", dbus_error.message);
        return bus_error_general;
    }
    ...
    if (dbus_error_is_set(&dbus_error)) {
        ...
        wifi_util_info_print(WIFI_BUS, "%s:%d dbus_bus_request_name failed with  %s:%d\n", __func__,
            __LINE__, dbus_error.message, ret);
        return bus_error_component_name_duplicate;
    }

    if (ret != DBUS_REQUEST_NAME_REPLY_PRIMARY_OWNER) {
        wifi_util_info_print(WIFI_BUS, "Dbus: not primary owner, ret = %d\n", ret);
        return bus_error_general;
    }
    ...
    if (!dbus_connection_register_object_path(p_dbus_handle, handle->dbus_path,
        ...
        wifi_util_info_print(WIFI_BUS, "Failed to register object path\n");
        return -1;
    }
    ...
    if (handle->subscribe_callback == NULL) {
        wifi_util_info_print(WIFI_BUS, "%s:%d subscribe_callback falied\r\n", __func__, __LINE__);
        return bus_error_general;
    }
    wifi_util_info_print(WIFI_BUS, "In %s:%d\n", __func__, __LINE__);

    handle->method_callback = hash_map_create();
    if (handle->method_callback == NULL) {
        wifi_util_info_print(WIFI_BUS, "%s:%d method_callback falied\r\n", __func__, __LINE__);
        return bus_error_general;
    }
    wifi_util_info_print(WIFI_BUS, "In %s:%d\n", __func__, __LINE__);
...
static bus_error_t bus_close(bus_handle_t *handle)
{
    ...
    DBusConnection *p_dbus_handle = handle->u.dbus_handle;
    if (!p_dbus_handle) {
        wifi_util_info_print(WIFI_BUS, "dbus handler is NULL %s: %d\n", __func__, __LINE__);
        return bus_error_general;
    }
    wifi_util_info_print(WIFI_BUS, "%s:%d: bus:bus_close() is successful, handle:%p\n", __func__,
        __LINE__, p_dbus_handle);
    ...
    uint32_t traceParentLength, char *traceState, uint32_t traceStateLength)
{
    ...
    wifi_util_info_print(WIFI_BUS, "[DL]%s:%d traceparent : %s\n", __func__, __LINE__, traceParent);
    ...
    wifi_util_info_print(WIFI_BUS, "[DL]%s:%d traceState : %s\n", __func__, __LINE__, traceState);
...
)
{
    ...
    if (sub_register) {
        ...
        DBUS_MESSAGE_APPEND_STRING_1(&iter, comp_name);

        dbus_message_iter_append_basic(&iter, DBUS_TYPE_INT32, &component_version);
        DBUS_MESSAGE_APPEND_STRING_1(&iter, path);
        DBUS_MESSAGE_APPEND_STRING_1(&iter, subsystem_prefix);
        ...
        for (int i = 0; i < size; i++) {
            ...
            DBUS_MESSAGE_APPEND_STRING_1(&struct_iter, name_space[i].name_space);
        ...
        DBUS_MESSAGE_APPEND_STRING_1(&iter, handle->component_name);
        DBUS_MESSAGE_APPEND_STRING_1(&iter, name_space[0].name_space);
        wifi_util_info_print(WIFI_BUS, "%s: at %d\n", __func__, __LINE__);
    }
    ...
    if (reply == NULL) {
        wifi_util_info_print(WIFI_BUS, "Reply Null\n");
        dbus_message_unref(message);
    ...
    char **comp)
{
    ...
    dbus_message_iter_init_append(message, &iter);
    DBUS_MESSAGE_APPEND_STRING_1(&iter, namespace);
    DBUS_MESSAGE_APPEND_STRING_1(&iter, subsystem_prefix);
    reply = dbus_connection_send_with_reply_and_block(p_dbus_handle, message, -1, &dbus_error);

    if (dbus_error_is_set(&dbus_error)) {
        wifi_util_info_print(WIFI_BUS, "Error sending message: %s\n", dbus_error.message);
        dbus_error_free(&dbus_error);
        dbus_message_unref(message);
        return bus_error_general;
    }
        ...
        if (dbus_message_iter_get_arg_type(&iter) == DBUS_TYPE_ARRAY) {
            ...
            while (dbus_message_iter_get_arg_type(&array_iter) != DBUS_TYPE_INVALID) {
                ...
                if (dbus_message_iter_get_arg_type(&array_iter) == DBUS_TYPE_STRUCT) {
                    ...
                    wifi_util_info_print(WIFI_BUS, "Expected DBUS_TYPE_STRUCT, found %d instead.\n",
                        dbus_message_iter_get_arg_type(&array_iter));
                    return bus_error_general;
                }
            }
        } else {
            wifi_util_info_print(WIFI_BUS, "Expected DBUS_TYPE_ARRAY, found %d instead.\n",
                dbus_message_iter_get_arg_type(&iter));
        }
...
static bus_error_t bus_set(bus_handle_t *handle, char const *name, raw_data_t *data)
{
    ...
    dbus_message_iter_open_container(&array_iter, DBUS_TYPE_STRUCT, NULL, &struct_iter);
    DBUS_MESSAGE_APPEND_STRING_1(&struct_iter, name);
    dbus_message_iter_append_basic(&struct_iter, DBUS_TYPE_STRING, &int_str);
    dbus_message_iter_append_basic(&struct_iter, DBUS_TYPE_INT32, &type);
    dbus_message_iter_close_container(&array_iter, &struct_iter);
    dbus_message_iter_close_container(&iter, &array_iter);
...
static bus_error_t bus_data_get(bus_handle_t *handle, char const *name, raw_data_t *data)
{
    ...
    DBUS_MESSAGE_APPEND_STRING_1(&array_iter, name);
    ...
    if (reply == NULL) {
        wifi_util_info_print(WIFI_BUS, "Reply is Nulli Line=%d\n", __LINE__);
        dbus_message_unref(message);
        return bus_error_destination_response_failure;
    }
    ...
    if (!val || !val->parameterValue) {
        ...
        wifi_util_info_print(WIFI_BUS, "Reply doesnt have value\n");
        return bus_error_general;
    }
    ...
    if ((data->data_type == bus_data_type_bytes || data->data_type == bus_data_type_string) &&
        ...
        if (data->raw_data.bytes == NULL) {
            wifi_util_info_print(WIFI_BUS, "%s:%d: bus: memory alloc is failed:%d for name:%s\n",
...
static bus_error_t bus_event_publish(bus_handle_t *handle, char const *name, raw_data_t *data)
{
    ...
    if (!p_dbus_handle) {
        wifi_util_info_print(WIFI_BUS, "%s:%d bus handler is not valid\n", __func__, __LINE__);
        return bus_error_general;
    }
    ...
    case bus_data_type_uint32:
        ...
        break;
    default:
        wifi_util_info_print(WIFI_BUS, "%s:%d: bus: Invalid data_type:%d for name:%s.\n", __func__,
    ...
    char const *param_str)
{
    ...
    dbus_message_iter_open_container(&array_iter, DBUS_TYPE_STRUCT, NULL, &struct_iter);
    tmp = ccsp_string;
    DBUS_MESSAGE_APPEND_STRING_1(&struct_iter, param_name);
    ...
    if (type == DBUS_MESSAGE_TYPE_METHOD_RETURN) {
        ...
        wifi_util_info_print(WIFI_BUS, "Failure return\n");
    ...
    bus_data_element_t *data_element)
{
    ...
    if (element_namespace == NULL) {
        wifi_util_info_print(WIFI_BUS, "%s:%d calloc for namespace is failed\n", __func__,
            __LINE__);
        return bus_error_out_of_resources;
    }
    ...
    raw_data_t *input_data, raw_data_t *output_data, uint8_t input_bus_data)
{
    ...
    case BUS_METHOD_SET_GET:
        ...
        if (strcmp(event, "Device.WiFi.AP.STA.GetAssocRequest") == 0) {
            ...
            dbus_message_iter_open_container(&array_iter, DBUS_TYPE_STRUCT, NULL, &struct_iter);
            DBUS_MESSAGE_APPEND_STRING_1(&struct_iter, event);
            dbus_message_iter_append_basic(&struct_iter, DBUS_TYPE_STRING, &int_str);
            dbus_message_iter_append_basic(&struct_iter, DBUS_TYPE_INT32, &type);
            dbus_message_iter_close_container(&array_iter, &struct_iter);
            dbus_message_iter_close_container(&iter, &array_iter);
            ...
            if ((output_data->data_type == bus_data_type_bytes ||
                ...
                if (output_data->raw_data.bytes == NULL) {
                    wifi_util_info_print(WIFI_BUS, "%s:%d: bus: memory alloc is failed\n");
                    return bus_error_out_of_resources;
                }

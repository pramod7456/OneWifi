...
quality_flags_t linkq_t::m_quality_flag = { false, false, false, false, false, false };
                                ...
                                bool update_alarm)
{
    ...
    if (m_quality_flag.aggregate) {
        wifi_util_error_print(WIFI_APPS,"In Aggregte %s:%d\n",__func__,__LINE__);
        cnt = 0;
        ...
        wifi_util_error_print(WIFI_APPS,"%s:%dNot In Aggregte\n",__func__,__LINE__);
        v.m_val[0].m_re = 0;
        v.m_val[1].m_re = 0;
        v.m_val[2].m_re = 0;
        v.m_val[11].m_re = 0;
    ...
    // -------------------------------------------------
    m_sampled++;
    wifi_util_error_print(WIFI_APPS,"%s:%dPramod aggscore = %f,downlink score=%f uplinkscore=%f\n",__func__,__LINE__,v.m_val[11].m_re,v.m_val[9].m_re,v.m_val[10].m_re);
    
    if ( (m_quality_flag.aggregate) && v.m_val[11].m_re < m_threshold) {
    ...
    if (update_alarm) {
        wifi_util_info_print(WIFI_APPS,"Pramod in update alarm\n");
        alarm = (m_threshold_cross_counter >= ceil(0.8 * m_sampled));
        wifi_util_info_print(WIFI_APPS,"Pramod in update alarm = %d counter=%d and threshold=%f\n",alarm,m_threshold_cross_counter,m_threshold);
        m_alarm = alarm;
        m_sampled = 0;
...
void linkq_t::update_window_per()
{
    ...
    if (last.err_recv >= first.err_recv)
     ...
     pthread_mutex_unlock(&m_deque_lock);
    wifi_util_info_print(WIFI_APPS,"DOWNLINK_PER last.pkt_sent=%d first.pkt_sent=%d sent_delta=%d err_sent_delta=%d\n",last.pkt_sent,first.pkt_sent,sent_delta,err_sent_delta);
    if (sent_delta > 0)
...
int linkq_t::init(double threshold, unsigned int reporting_mult, stats_arg_t *stats )//, const char *test_file_name)
{
    ...
    wifi_util_info_print(WIFI_APPS,"DOWNLINK_PER sample.pkt_sent=%dsample.err_sent=%d\n",sample.pkt_sent,sample.err_sent);
    // Push latest sample

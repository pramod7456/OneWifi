export function renderLinkQualityChart(data) {
    ...
    for (let i = 0; i < data.Devices.length; i++) {
        ...
        const layout = {
                        title: { 
                            text: `Link Quality: ${dev.MAC} (${(Array.isArray(dev.LinkQuality.Score) ? dev.LinkQuality.Score[dev.LinkQuality.Score.length-1] : dev.LinkQuality.Score)?.toFixed ? (Array.isArray(dev.LinkQuality.Score) ? dev.LinkQuality.Score[dev.LinkQuality.Score.length-1] : dev.LinkQuality.Score).toFixed(2) : dev.LinkQuality.Score})`, 
                            pad: { t: 25 } 
                        },
